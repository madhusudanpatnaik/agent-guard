...
terminates TLS using a certificate signed by the AgentOps CA — which the operator
...
_log = logging.getLogger("agentops.proxy")
...
def _blocked(conn, decision) -> None:
    body = json.dumps({"agentops": "blocked", "decision": decision.decision,
...
def _forward_and_log(db, agent, method, url, resource, action, headers, body):
    ...
    try:
        with span("agentops.proxy.forward", **{"http.method": method, "agentops.resource": resource}), \
