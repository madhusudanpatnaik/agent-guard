...
certificate for the destination host, signed by the **AgentOps CA**. The operator
...
``AGENTOPS_PROXY_CA_DIR``). Leaf certs are minted on demand per host and cached.
"""
...
class ProxyCA:
    def __init__(self, ca_dir: str):
        ...
        self.cert_path = self.dir / "agentops-ca.crt"
        self.key_path = self.dir / "agentops-ca.key"
        self._leaf_dir = Path(tempfile.mkdtemp(prefix="agentops-leaf-"))
    ...
    def _load_or_create(self):
        ...
        name = x509.Name([
            x509.NameAttribute(NameOID.ORGANIZATION_NAME, "AgentOps"),
            x509.NameAttribute(NameOID.COMMON_NAME, "AgentOps Proxy CA"),
        ])
