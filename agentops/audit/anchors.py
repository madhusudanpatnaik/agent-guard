...
* **file** (default) — append ``seq<TAB>hash`` to ``audit_anchor_path``. This is
  also what :func:`agentops.audit.ledger.verify_chain` reads to detect head
...
* **rfc3161** — obtain an RFC-3161 signed timestamp token from a Time-Stamp
  ...
  without trusting AgentOps. Tokens are written under ``<anchor>.tsr/``.
...
_log = logging.getLogger("agentops.audit.anchors")
    ...
    def anchor(self, seq: int, head_hash: str) -> None:
        ...
        try:
            ...
            resp = httpx.post(self.url, json={"seq": seq, "hash": head_hash,
                                              "kind": "agentops.ledger.head"}, timeout=3.0)
    ...
    for name in names:
        ...
        if n == "file":
            ...
            built.append(RFC3161Anchor(settings.rfc3161_tsa_url,
                                       (settings.audit_anchor_path or "./agentops-ledger-anchor")
                                       + ".tsr"))
        else:
