"""Central configuration for the AgentGuard control plane.
...
class Settings(BaseSettings):
    ...
    # --- Core -----------------------------------------------------------
    app_name: str = "AgentGuard Control Plane"
    ...
    bootstrap_admin_email: str = "admin@agentguard.local"
    ...
    oidc_redirect_uri: str = ""    # e.g. https://agentguard.corp/api/auth/oidc/callback
    ...
    audit_anchor_path: str = "./agentguard-ledger-anchor.log"
    ...
    # HMAC-SHA256 over the exact request body in an ``X-AgentGuard-Signature:
    ...
    proxy_ca_dir: str = "./agentguard-ca"
    ...
    policy_engine: str = ""
    policy_engine_url: str = ""   # e.g. http://localhost:8181/v1/data/agentguard/allow
    ...
    otel_service_name: str = "agentguard"
    ...
    # --- Logging --------------------------------------------------------
    log_level: str = "INFO"
    log_json: bool = True  # JSON-lines logs for the agentguard.* namespace
    ...
    def production_problems(self) -> list[str]:
        ...
        if self.secret_key in weak_secrets or len(self.secret_key) < 32:
            problems.append(
                "AGENTGUARD_SECRET_KEY must be a strong random value (>=32 chars) in production"
            )
        if self.bootstrap_admin_password in {"admin", ""}:
            problems.append(
                "AGENTGUARD_BOOTSTRAP_ADMIN_PASSWORD must be changed from the default in production"
            )
        return problems
