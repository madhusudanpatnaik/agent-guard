...
The built-in :class:`~agentguard.policy.engine.PolicyEngine` is fast, pure, and
...
while AgentGuard still owns everything around it — DLP exfiltration blocking,
the audit ledger, approvals, alerts, quotas.

* **OPA** (``policy_engine=opa``): POSTs ``{"input": <decision-input>}`` to the
  configured OPA data URL (e.g. ``/v1/data/agentguard/authz``) and reads back
...
_log = logging.getLogger("agentguard.policy.external")
