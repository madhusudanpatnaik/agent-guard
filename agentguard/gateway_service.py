...
def _recent_action_count(db: Session, agent_id: int, seconds: int) -> int:
    ...
    Routed through the configured :mod:`agentguard.counters` backend (DB by
...
def authorize_action(
    ...
    with span("agentguard.dlp.scan", **{"agentguard.resource": req.resource}):
    ...
    if quota_used >= agent.quota:
        ...
        if external is not None:
            # Policy-as-code: OPA/Cedar owns the RBAC verdict; AgentGuard still
...
def execute_action(
    ...
    *,
    ...
    try:
        with span("agentguard.upstream.call",
                  **{"http.method": method, "agentguard.connector": connector.name}), \
