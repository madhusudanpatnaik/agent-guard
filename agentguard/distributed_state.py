...
_log = logging.getLogger("agentguard.distributed_state")
...
def redis_should_dispatch(client, key: str, window: int) -> bool | None:
    ...
    """
    try:
        claimed = client.set(f"aguard:throttle:{key}", "1", nx=True, ex=max(window, 1))
        return bool(claimed)
    except Exception as exc:  # noqa: BLE001
...
def redis_get_detectors(client, org_id: int | None) -> list[dict] | None:
    ...
    try:
        raw = client.get(f"aguard:detectors:{org_id if org_id is not None else '_none'}")
...
def redis_set_detectors(client, org_id: int | None, specs: list[dict], ttl: float) -> None:
    ...
    try:
        client.set(f"aguard:detectors:{org_id if org_id is not None else '_none'}",
...
def redis_invalidate_detectors(client, org_id: int | None) -> None:
    ...
    try:
        if org_id is None:
            keys = client.keys("aguard:detectors:*")
            if keys:
                client.delete(*keys)
        else:
            client.delete(f"aguard:detectors:{org_id}")
